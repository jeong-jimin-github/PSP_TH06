...
<p align="center">
  <img src="assets/gameplay_cut1.png" width="32%" alt="ゲームタイトル" />
  <img src="assets/gameplay_cut2.png" width="32%" alt="ステージ開始" />
  <img src="assets/gameplay_cut3.png" width="32%" alt="インゲームプレイ" />
</p>
...
- 元の640×480画面を正確に1/2の320×240で描画し、PSPの480×272画面中央に
  黒い余白を配置します（ゲーム内設定で4:3維持と16:9フルスクリーンの切り替えが可能です）。
- 32 MiBのPSP-1000向けにテクスチャ、ストリーミング、メモリ使用量を調整しています。
- PSP-1000の333/166 MHzクロックで60 Hzを目標に、スプライトバッチ、VFPU演算（ZunMath関数のインライン化）、
  弾幕（Bullet）の演算と配列構造の改善、オーディオミキシングを最適化しています。
...
- PSPで不要なMIDI、ウィンドウモード、色深度の設定は非表示です。音楽はWAVまたは
  Offを選べ、画面アスペクト比（4:3フィット vs 16:9引き伸ばし）の設定が追加されました。
- 設定、スコア、セーブ、リプレイはEBOOTと同じ場所に書き込まれます。ディレクトリを
  書き込み可能にしてください。
- ゲームプレイからメインメニューに戻る際、画面が正しく描画されないディスプレイの問題を解決しました。
- `loadtrace.txt`は起動ごとにEBOOTの隣へ作り直され、最後に完了したステージ読み込み
  手順を記録します。

## 開発用ビルド

WSL環境やmacOS環境でPSPターゲットを直接ビルドすることもできます。
...
macOS環境では、提供されているビルドスクリプトを使用して依存関係の設定とコンパイルを簡単に行うことができます。
